...
=head1 NOTE ON TERM::READLINE

Although L<Term::CLI> has been created to work with L<Term::ReadLine> in
general, it works best when either L<Term::ReadLine::Gnu>(3p), or
L<Term::ReadLine::Perl>(3p) is also present. If not, then L<Term::ReadLine>
will load a "stub" interface (C<Term::ReadLine::Stub>), which supports neither
command line editing nor completion.
...
If you run any of the previous examples and enter I<Ctrl-\> or
I<Ctrl-Z>, you'll notice that the program quits or suspends.

This may not be what you want, though (especially an accidental
C<QUIT> signal can be annoying).

You can easily set those signals to C<IGNORE> like we did with C<INT>,
but you may not necessarily want to do that; perhaps you just want to
prevent these signals to be accidentally generated from the keyboard,
but still allow the reception of the signals themselves.

Fortunately, there is a way to do this, via the C<ignore_keyboard_signals>
attribute:
...
(If you want to know why it's not a good idea to disable the C<INT>
key signal, see
L<ignore_keyboard_signals in Term::CLI::ReadLine|Term::CLI::ReadLine/ignore_keyboard_signals>.)

=head2 A C<suspend> command

Since we just disabled suspension via I<Ctrl-Z>, we should
add a C<suspend> command:
...
Enter I<Ctrl-Z>; this should suspend the program. Type C<fg> + I<ENTER> to
continue again.

=item 3.

Enter I<Ctrl-\>; this should kill the program.
...
perl tutorial/example_17_signals.pl
...
Enter C<fg> + I<ENTER> to continue again.

=item 4.

Enter I<Ctrl-\>; this should not have any effect.
...
For completeness' sake, here are a number of alternative options:
...
instead of C<exit>, in other places of your program. Some of these places may
not have access to a valid L<Term::CLI> or L<Term::CLI::CommandSet> object, so 
...
The above not have the desired effect of writing the history to file upon
exit.
