...
L<Term::CLI>'s C<add_command> method.
    ...
    $term->add_command, Term::CLI::Command->new(
        ...
    );
...
This is the method we'll use for this tutorial, and comes in
handy further down the line.
...
and C<description> attributes in the C<exti> command definition:
...
With the above two additions, we have:
    ...
    bash$ perl tutorial/example_13_sub_cmd_and_args.pl
...
usage line is too long (it lists every possible sub-command):
    ...
    bash$ perl tutorial/example_14_debug_command.pl
...
The following commands should be allowed now:
    ...
    bash$ perl tutorial/example_15_options.pl
...
By default, the L<Term::CLI> objects to not try to read or write
